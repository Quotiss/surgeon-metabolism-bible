...
const CaseStudy = () => {
  ...
  const [expandedStates, setExpandedStates] = useState<boolean[]>(new Array(testimonials.length).fill(false));
  ...
  return <div className="mt-12 sm:mt-16 md:mt-20">
      ...
      <div className="relative max-w-4xl mx-auto">
                  ...
                  <CardContent className="p-0">
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                      <div className="flex-shrink-0">
                        <img src={testimonial.image} alt={`${testimonial.name} - ${testimonial.specialty}`} className="w-20 h-20 md:w-24 md:h-24 rounded-full object-cover border-4 border-blue-100" />
                      </div>
                      <div className="flex-1">
                        <div className="mb-4">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className="flex gap-0.5">
                              {[...Array(5)].map((_, starIndex) => <Star key={starIndex} className="h-4 w-4 fill-yellow-400 text-yellow-400" />)}
                            </div>
                          </div>
                        ...
                        <div className="text-slate-700 leading-relaxed">
                          ...
                          {expandedStates[index] && <div className="mt-4 p-4 bg-slate-50 rounded-lg border-l-4 border-blue-500">
                              <h4 className="font-semibold text-slate-900 mb-3">Full Story:</h4>
                              <div className="text-slate-700 leading-relaxed space-y-4">
                                {testimonial.fullContent.split('\n\n').map((paragraph, pIndex) => <p key={pIndex} className="text-sm sm:text-base">
                                    {paragraph}
                                  </p>)}
                              </div>
                            </div>}
                          ...
                          {/* Toggle Button */}
                          <button onClick={() => toggleExpanded(index)} className="mt-4 flex items-center gap-2 text-blue-600 hover:text-blue-700 font-medium text-sm transition-colors">
        ...
        <div className="flex justify-center space-x-2 mt-6">
          {testimonials.map((_, index) => <button key={index} className={`w-2 h-2 rounded-full transition-colors ${index === current ? "bg-blue-600" : "bg-slate-300"}`} onClick={() => api?.scrollTo(index)} />)}
        </div>
      </div>
    </div>;
};
